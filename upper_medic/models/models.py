...
class Contracts(models.Model):
    ...
    creator_id = fields.Many2one('upper_medic_employee')
...
class Job(models.Model):
    _name = 'upper_medic_job'
    _description = 'Jobs'
    name = fields.Char()


class Employees(models.Model):
    _name = 'upper_medic_employee'
    ...
    job_id = fields.Many2one('upper_medic_job')
    department_id = fields.Many2one('upper_medic_department')
    company = fields.Selection(selection=[('upper', 'Upper Medic'), ('baltoe', 'Baltoe'), ('common', 'Common'), ], )
...
class AccGuide(models.Model):
    ...
    parent_id = fields.Many2one('upper_medic_accguide')


class Income(models.Model):
    _name = 'upper_medic_income'
    _description = 'Income'

    creation_date = fields.Date()
    due_date = fields.Date()
    contract_id = fields.Many2one('upper_medic_contract')
    ...
    company_account_id = fields.Many2one('upper_medic_accguide')
    doctype = fields.Many2one('upper_medic_doctype')
    company = fields.Selection(selection=[('upper', 'Upper Medic'), ('baltoe', 'Baltoe')], )
    active = fields.Boolean(default=True)


class CostCenter(models.Model):
    _name = 'upper_medic_costcenter'
...
class JE(models.Model):
    _name = 'upper_medic_je'
    _description = 'JE'
    _rec_name = 'explain'
    explain = fields.Char('explain')
    out_value = fields.Float()
    in_value = fields.Float()
    costcenter_id = fields.Many2one('upper_medic_costcenter')
    creation_date = fields.Date()
    due_date = fields.Date()
    contract_id = fields.Many2one('upper_medic_contract')
    customer_id = fields.Many2one(related='contract_id.customer_id')
    ...
    doctype = fields.Many2one('upper_medic_doctype')
    company_account_id = fields.Many2one('upper_medic_accguide')
    on_account = fields.Many2one('upper_medic_accguide')
    att_files = fields.Binary(string="Attachments", )
    active = fields.Boolean(default=True)
